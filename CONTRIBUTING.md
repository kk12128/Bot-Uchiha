# Contribuindo para o Bot-Uchiha
...
2. **Clone o Repositório**: Clone seu fork localmente:
   ```bash
   git clone https://github.com/seu_usuario/Bot-Uchiha.git
   cd Bot-Uchiha
   ```
3. **Crie uma Branch**: Crie uma branch para sua nova funcionalidade ou correção de bug:
   ```bash
   git checkout -b nome-da-sua-branch
   ```
4. **Faça Alterações**: Realize as modificações desejadas. Teste suas alterações para garantir que tudo funcione corretamente.
5. **Adicione e Faça Commit das Mudanças**: Adicione suas alterações e faça commit:
   ```bash
   git add .
   git commit -m "Descrição clara das suas mudanças"
   ```
6. **Envie para o Seu Fork**: Envie suas alterações para o seu fork:
   ```bash
